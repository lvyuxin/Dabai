...
public class ItemIoDeviceAdapter extends BaseQuickAdapter<IoBean.DeviceOrSceneInfoBean,BaseViewHolder> {
    public ItemIoDeviceAdapter(int layoutResId, @Nullable List<IoBean.DeviceOrSceneInfoBean> data) {
        super(layoutResId, data);
    }

    @Override
    protected void convert(BaseViewHolder helper, IoBean.DeviceOrSceneInfoBean item) {
        helper.setText(R.id.tv_name, item.getDevice_or_scene_name());
        ...
        if (item.getControl_type().equals(ContentStr.Control_type.humanFeeling)) {
            ...
            if (item.getIotype() == Content.TYPE.TRIGGER) {
                tvState.setText("按下");
            } else if (item.getIotype() ==  Content.TYPE.NOT_TRIGGER) {
                tvState.setText("释放");
