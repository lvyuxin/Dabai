...
public class ConfigFragment extends BaseFragment {
    ...
    private List<ReadJson.AlldeviceBean.DataBean> curtainList = new ArrayList<>();
    ...
    private void goDeviceDetail() {
        ...
        adapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                ...
                switch (position) {
                    case 0:
                        String ipinfor = readJson.getAlldevice().getIpinfor();
                        bundle.putString("ipinfor", ipinfor);
                        startActivity(IntelligentHostActivity.class, bundle);
                        break;
                    case 1:
                    ...
                    case 5:
                        if (curtainList.size() > 0) {
                            bundle.putSerializable("curtainList", (Serializable) curtainList);
    ...
    private void addNames() {
        ...
        list.add(5, new ConfigSettingBean("窗帘模块", 0));
        list.add(6, new ConfigSettingBean("扩展模块", 0));
        list.add(7, new ConfigSettingBean("io模块", 0));
        list.add(8, new ConfigSettingBean("环境盒子", 0));
        list.add(9, new ConfigSettingBean("空调模块", 0));
        list.add(10, new ConfigSettingBean("视屏监控模块", 0));
    }
    ...
    private void classifyLocalDevice(ReadJson.AlldeviceBean alldevice) {
        if (alldevice.getData() != null) {
            ...
            list.get(5).setDeviceCount(curtainList.size());
            list.get(6).setDeviceCount(extendedList.size());
            list.get(7).setDeviceCount(ioList.size());
            list.get(8).setDeviceCount(environmentBoxList.size());
        }
    }
